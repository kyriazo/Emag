...
class LayoutBuilderDisableInteractionsTest extends WebDriverTestBase {
  ...
  protected function setUp() {
    ...
    BlockContent::create([
      'type' => 'basic',
      'info' => 'Block with hardcoded form',
      'body' => [
        // Add hard-coded form.
        'value' => '<form id="block-with-hardcoded-form">First name:<br /><input name="firstname" type="text" /><br/>Last name:<br/><input name="lastname" type="text"/><br /><input type="submit" value="Submit" /></form>',
        'format' => 'full_html',
      ],
    ])->save();
  }
  ...
  /**
   * Tests that blocks with forms do not prevent saving.
   *
   * @dataProvider providerTestBlocksWithFormsAllowSave
   */
  public function testBlocksWithFormsAllowSave($block_link_text, $rendered_locator) {
    $assert_session = $this->assertSession();
    $page = $this->getSession()->getPage();
    ...
    $this->drupalGet('node/1/layout');
    $this->addBlock($block_link_text, $rendered_locator);
  ...
  /**
   * Provides data for ::testBlocksWithFormsAllowSave().
   */
  public function providerTestBlocksWithFormsAllowSave() {
    $data[] = [
      'Block with hardcoded form',
      '#block-with-hardcoded-form',
    ];
    $data[] = [
      'User login',
      '#layout-builder .block-user',
    ];
    $data[] = [
      'Search form',
    ...
    $data[] = [
      'Test Exposed Filter',
      '#layout-builder .block-views-blocktest-exposed-filter-block-1',
    ];
